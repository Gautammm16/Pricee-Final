...
const Oursingleproduct = ({isopen,setisopen}) => {
  const [data,setData ] = useState(null)
  const location = useLocation()
  const param = useParams()
  const getProductData = async()=>{
    const fetchdata = await fetch(`${window.path}/getoursingleproduct/${param.id}`,{
      method:"get"
    })
              ...
              <div x-data="{ image: 1 }" x-cloak>
                <div class="h-64 md:h-80 rounded-lg bg-white mb-4">
                  <div  class="h-64  md:h-80 rounded-lg  mb-4 flex items-center justify-center">
                    <img class="text-5xl"  src={data.images[0]} />
                  </div>
                <div className='flex'>
              ...
              <div class="flex items-center space-x-4 my-4">
                <div>
                  <div class="rounded-lg bg-gray-100 flex py-2 px-3">
                    ...
                    <span class="font-bold text-indigo-600 text-3xl">{data.discounted_price}</span>
                  </div>
                </div>
                <div class="flex-1">
                  <p class="text-green-500 text-xl font-semibold">Save {data.discount}%</p>
...
export default Oursingleproduct